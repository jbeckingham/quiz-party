...
const MainPanel = ({ gameState, myName, onAnswerSubmitted, onQuestionSubmitted, onResultsSubmitted}) => {
    ...
    const allAnswersSubmitted = gameState.players.length - 1 === Object.keys(gameState.currentQuestion.answers).length
    ...
    if (!answered && !questionAskedByMe) {
        ...
        return <AnswersMarkingView gameState={gameState} onResultsSubmitted={onResultsSubmitted} />
    }

    return <AnswersPendingView gameState={gameState} />
}

const QuizView = ({ gameState, myName, onAnswerSubmitted, onQuestionSubmitted, onResultsSubmitted}) => (
    <Grid columns={2}>
        <Grid.Row>
